...
<script setup>
  ...
  const fetchList = async page => {
    ...
    try {
      const resp = await fetch(
        `http://192.168.1.10:8080/api/admin/detectives?page=${
          query.page
  ...
  const approve = async row => {
    await post(
      `http://192.168.1.10:8080/api/admin/detectives/${row.id}/approve`
    )
    fetchList()
  }
  ...
  const disableDetective = async row => {
    await post(
      `http://192.168.1.10:8080/api/admin/detectives/${row.id}/disable`
    )
    fetchList()
  }
  const view = row => {
  ...
  const submitReject = async () => {
    ...
    await post(
      `http://192.168.1.10:8080/api/admin/detectives/${review.id}/reject`,
