    ...
    <u-form :model="form" ref="formRef" class="form-wrap" :labelWidth="90">
      ...
      <view class="section">
        ...
        <u-form-item label="团队人数" prop="teamSize" required>
          <u-input v-model.number="form.teamSize" type="number" placeholder="请输入人数" />
        </u-form-item>
        <u-form-item label="设备清单" prop="devices" required>
          <u-checkbox-group v-model="form.devices">
            <u-checkbox v-for="d in deviceOptions" :key="d" :name="d" :label="d" />
          </u-checkbox-group>
        </u-form-item>
        <u-form-item label="设备照片" prop="devicePhotos" required>
          <view class="upload-grid">
            <view v-for="(img, idx) in deviceImages" :key="idx" class="img-item">
              <image :src="img.url" mode="aspectFill" />
            </view>
            <view v-if="deviceImages.length < 6" class="img-item add" @click="chooseDeviceImage">
              <view class="inner"><text class="plus">＋</text><text class="hint">添加照片</text></view>
            </view>
          </view>
        </u-form-item>
      </view>
      ...
      <view class="section">
        ...
        <u-form-item label="经验年限" prop="experienceYears" required>
          <u-input v-model.number="form.experienceYears" type="number" placeholder="如：3" />
        </u-form-item>
        <u-form-item label="服务范围" prop="serviceAreas" required>
          <u-input v-model="serviceAreasText" placeholder="多城市用顿号分隔，如：上海、苏州" @blur="onAreasBlur" />
        </u-form-item>
        <u-form-item label="可服务时间" prop="availableTimes" required>
          <u-checkbox-group v-model="form.availableTimes">
            <u-checkbox v-for="t in timeOptions" :key="t" :name="t" :label="t" />
          </u-checkbox-group>
        </u-form-item>
        <u-form-item label="个人简介" prop="bio">
          <u-textarea v-model="form.bio" placeholder="选填：擅长领域、经验介绍" autoHeight />
        </u-form-item>
      </view>
      ...
      <view class="section">
        ...
        <u-form-item label="身份证正面" prop="idCardFront" required>
          ...
          <image v-if="form.idCardFront" class="thumb" :src="display(form.idCardFront)" />
        </u-form-item>
        <u-form-item label="身份证反面" prop="idCardBack" required>
          <u-button size="small" @click="pickId('back')">上传图片</u-button>
          <image v-if="form.idCardBack" class="thumb" :src="display(form.idCardBack)" />
        </u-form-item>
      ...
      <view class="section">
        ...
        <u-form-item label="收款账号" prop="payout.account" required>
          <u-input v-model="form.payout.account" placeholder="请输入微信号或支付宝账号" />
        </u-form-item>
        <u-form-item label="同意协议" prop="agree" required>
          <u-checkbox v-model="form.agree" :name="'agree'" label="我已阅读并同意《侦探服务条款》《隐私政策》" />
        </u-form-item>
      </view>
    </u-form>

    <view class="bottom">
      <u-button type="primary" :disabled="!canSubmit" @click="submit" :customStyle="primaryStyle">提交申请</u-button>
    </view>
...
<script setup>
  ...
  const deviceOptions = ['无人机', '热成像仪', '夜视仪', '望远镜', '对讲机', '寻宠犬', '车辆', 'GPS 追踪器', '其他']
  ...
  const display = (path) => (typeof path === 'string' && path.startsWith('/uploads/') ? `${BASE_URL}${path}` : path)
  ...
  const pickCert = () => {
    uni.chooseImage({ count: 6 - certImages.value.length, success: async res => {
  ...
  const chooseDeviceImage = () => {
    uni.chooseImage({ count: 6 - deviceImages.value.length, success: async res => {
      for (const p of res.tempFilePaths) {
  ...
  function resolveUploadUrl(resp) {
    try {
      ...
      if (typeof resp === 'string') {
        try { return resolveUploadUrl(JSON.parse(resp)) } catch { return '' }
      }
  ...
  const primaryStyle = { background: 'linear-gradient(135deg, var(--brand-gradient-start), var(--brand-gradient-end))', border: 'none' }

  const canSubmit = computed(() => {
    const f = form
    return !!(
      f.realName && f.phone && f.city && f.teamSize && f.devices?.length &&
      f.devicePhotos?.length && f.experienceYears && f.serviceAreas?.length &&
      f.availableTimes?.length && f.idCardFront && f.idCardBack &&
      f.payout?.type && f.payout?.account && f.agree
    )
  })
  ...
  const submit = async () => {
    try {
      ...
      const res = await request({ url: API.detective.apply, method: 'POST', data: payload })
...
<style lang="scss" scoped>
  .apply-page { background:#f7f8fa; min-height:100vh; padding-bottom:80px; }
  .header { display:flex; align-items:center; gap:12px; padding:16px 16px 8px; color:#fff;
    background: linear-gradient(135deg, var(--brand-gradient-start), var(--brand-gradient-end)); }
  .emoji{ font-size:30px; }
  .title{ font-size:18px; font-weight:700; display:block; }
  .sub{ font-size:12px; opacity:.9; }
  .form-wrap{ padding:8px 12px; }
  .section{ background:#fff; border-radius:12px; padding:8px 12px 2px; margin:10px 0; box-shadow:0 4px 16px rgba(91,143,249,.08); }
  .section-title{ font-weight:700; color:#333; padding:8px 2px 10px; }
  .upload-grid{ display:grid; grid-template-columns:repeat(4,1fr); gap:10px; padding:6px 0 12px; }
  .upload-grid.small{ grid-template-columns:repeat(5,1fr); }
  .img-item{ width:100%; padding-top:100%; position:relative; border-radius:12px; overflow:hidden; background:#f2f3f5; border:1px dashed #e5e6eb; display:flex; align-items:center; justify-content:center; }
  .img-item image{ position:absolute; inset:0; width:100%; height:100%; }
  .img-item.add{ color:#999; }
  .img-item.add .inner{ display:flex; flex-direction:column; align-items:center; justify-content:center; position:absolute; inset:0; }
  .plus{ font-size:28px; line-height:1; }
  .hint{ font-size:12px; }
  .thumb{ width:80px; height:60px; margin-left:10px; border-radius:6px; border:1px solid #eee; }
  .bottom{ position:fixed; left:0; right:0; bottom:env(safe-area-inset-bottom,0); padding:10px 14px; background:#fff; box-shadow:0 -2px 8px rgba(0,0,0,.06); }
</style>
